...
class AtomicAdd: public ForEachKernel<StridePolicy, CKT_cuda_blocksize, false> 
{
public:
  explicit AtomicAdd(int N): ForEachKernel<StridePolicy, CKT_cuda_blocksize, false>(N, "AtomicAdd"){
  }
};
...
TEST_CASE( "ForEachStride", "[sum]" ) {
  ...
  ForEachKernel<StridePolicy, CKT_cuda_blocksize, false> kernel(n1, "AtomicAdd");
